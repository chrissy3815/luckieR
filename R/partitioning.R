...
#'
#' @param Umat The survival/growth transition matrix: Umat\[i,j\] is the probability
...
#' @param maxAge The maximum age an individual can attain.  Optional. The
#'   default value is 100.
#' @param esR1 The 1st order reward matrix for the extended state space (see
#'   Details).  Optional.  The default assumes Poisson-distributed clutch sizes.
#' @param esR2 The 2nd order reward matrix for the extended state space (see
#'   Details).  Optional.  The default assumes Poisson-distributed clutch sizes.
#' @param esR3 The 3rd order reward matrix for the extended state space (see
#'   Details).  Optional.  The default assumes Poisson-distributed clutch sizes.
#' @param bsR1 The 1st order reward matrix for the extended state space that
#'   includes the pre-birth "ur-state" (see Details). Optional.  The default
#'   assumes Poisson-distributed clutch sizes.
#' @param bsR2 The 2nd order reward matrix for the extended state space that
#'   includes the pre-birth "ur-state" (see Details). Optional.  The default
#'   assumes Poisson-distributed clutch sizes.
#' @param bsR3 The 3rd order reward matrix for the extended state space that
#'   includes the pre-birth "ur-state" (see Details). Optional.  The default
#'   assumes Poisson-distributed clutch sizes.
#' @param survThreshold The threshold to use in determining lifespan (see return
...
#'
#' @details A pre-breeding census is assumed: reproduction happens before
...
#' * birthStateVar: the contribution to
#'   Var(LRO) from birth state luck
#' * survTrajecVar: a vector whose jth entry
...
#' * fecVar: a vector
#'   whose jth entry contains the contribution to Var(LRO) from fecundity luck
#'   at age j-1.
#' * totVar: the total variance in LRO
#' * birthStateSkewness: the
#'   contribution to LRO skewness from birth state luck
#' * survTrajecSkewness: a
#'   vector whose jth entry contains the contribution to LRO skewness from
#'   survival trajectory luck at age j-1.
#' * growthTrajecSkewness: a vector
#'   whose jth entry contains the contribution to LRO skewness from growth
#'   trajectory luck at age j-1.
#' * fecSkewness: a vector whose jth entry
#'   contains the contribution to LRO skewness from fecundity luck at age j-1.
#' * totSkewness: the total skewness in LRO
#' * lifespan: the age by which a
#'   proportion survThreshold of a cohort will be dead
#' @export
#'
#' @examples
...
partitionVarSkewnessNoEnvVar = function (Umat, Fmat, c0, maxAge=100,
                                         survThreshold=0.05,
                                         Fdist="Poisson",
                                         esR1=NULL, esR2=NULL,
                                         esR3=NULL,
  ...
  ## Sanity check input
  if (length(c0) != mz)
    stop ("Length of c0 does not match dimension of Umat")
  ...
  ## Sanity check
  epsilon = 0.00001
  if (sum(abs(range (Gbullet%*%Sbullet-Umat))) > epsilon)
    warning ("Gbullet %*% Sbullet does not equal Umat.")
  ...
  for (a in 2:maxAge) {
    ...
    growthUpdateVar[a] = esMu2Vec %*% c(Umat %*% PaC0, mzZero, mzZero)
...
#'   populations." The American Naturalist 204(2) and its online supplemnt.
#'
#' @return A list containing the following: * birthStateVar: the contribution to
...
#'   * birthEnvSkewness: the contribution to LRO skewness from birth environment
...
#'
#' @param PlistAllTraits A list of lists of survival/growth transition matrices.
...
#' @param c0 A vector specifying the offspring state distribution: c0\[j\] is
#'   the probability that an individual is born in state j
#' @param traitDist A vector whose jth entry is the probability of having trait
#'   j
#' @param maxAge The maximum age an individual can attain.  Optional. The
#'   default value is 100.
#' @param survThreshold The threshold to use in determining lifespan (see return
...
#' @param Fdist The clutch size distribution.  The recognized options are
#'   "Poisson" and "Bernoulli".  Optional.  The default value is "Poisson".
#' @param postBreedingCensus Set this to TRUE to assume a post-breeding census
...
#'
#' Partitions variance and 3rd central moment of lifetime reproductive
#' output (LRO) for models without trait or environmental variation.
#'
#' @param Umat The survival/growth transition matrix: Umat\[i,j\] is the probability
#'   of transitioning from state j to state i.
#' @param Fmat The fecundity matrix: Fmat\[i,j\] is the expected number of state i
...
#' * totVar: the total variance in LRO
#' * birthStateMu3: the
#'   contribution to LRO mu3 from birth state luck
#' * survTrajecMu3: a
#'   vector whose jth entry contains the contribution to LRO mu3 from
#'   survival trajectory luck at age j-1.
#' * growthTrajecMu3: a vector
#'   whose jth entry contains the contribution to LRO mu3 from growth
#'   trajectory luck at age j-1.
#' * fecMu3: a vector whose jth entry
#'   contains the contribution to LRO mu3 from fecundity luck at
#'   age j-1.
...
partitionVarMu3NoEnvVar = function (Umat, Fmat, c0, maxAge=100,
                                         survThreshold=0.05,
                                         Fdist="Poisson",
                                         esR1=NULL, esR2=NULL,
                                         esR3=NULL,
  ...
  for (a in 2:maxAge) {
    ...
    ## fecundity mu3 and var.
    fecMu3[a] = growthUpdateMu3[a-1] - fecUpdateMu3[a]
    fecVar[a] = growthUpdateVar[a-1] - fecUpdateVar[a]

    ## survival trajectory mu3 and var.
    survTrajecMu3[a-1] = fecUpdateMu3[a-1] -
      survUpdateMu3[a-1]
    survTrajecVar[a-1] = fecUpdateVar[a-1] - survUpdateVar[a-1]

    ## growth trajectory mu3 and var.
  ...
  ## Sanity check
  foo = totSurvTrajecMu3 + totGrowthTrajecMu3 +
    totFecMu3 + birthStateMu3
  if ((foo < (1 - percentTol)*totMu3) |
      (foo > (1 + percentTol)*totMu3))
    warning("Mu3 components do not sum to total mu3 as calculated by bsMu3Vec.")
...
#'
#' Partitions variance and mu3 of lifetime reproductive output (LRO) for
#' models with environmental variation but without trait variation.
#'
#' @param Plist A list of survival/growth transition matrices.
#'   Plist\[\[q\]\]\[i,j\] is the probability of transitioning from state j to
...
#' * birthEnvMu3: the contribution to LRO mu3 from birth environment
#'   luck
#' * survTrajecMu3: a vector whose jth entry contains the
#'   contribution to LRO mu3 from survival trajectory luck at age
#'   j-1.
#' * growthTrajecMu3: a vector whose jth entry contains the contribution to
#'   LRO mu3 from growth trajectory luck at age j-1.
#' * envTrajecMu3:
#'   a vector whose jth entry contains the contribution to LRO mu3 from
#'   environment trajectory luck at age j-1.
#' * fecMu3: a vector whose jth
#'   entry contains the contribution to LRO mu3 from fecundity luck at age
#'   j-1.
#' * totMu3: the total mu3 in LRO
#' * lifespan: the age by which
#'   a proportion survThreshold of a cohort will be dead
#' @importFrom Matrix bdiag
#' @export
#'
...
partitionVarMu3EnvVar = function (Plist, Flist, Q, c0,
                                       maxAge=100,
                                       survThreshold=0.05,
                                       Fdist="Poisson",
                                       esR1=NULL, esR2=NULL, esR3=NULL,
...
## Partitions variance and mu3 of LRO in the presence of both
## env. variation and trait variation.  Includes the contribution from
## trait variation.
...
#' * varFromTraits = the
#'   contribution to Var(LRO) from trait variation
#' * mu3FromTraits = the contribution to LRO mu3 from trait
#'   variation * totVar: the total variance in LRO
#' * totMu3: the total mu3 in LRO
#' * birthEnvVar: the contribution to Var(LRO)
#'   from birth environment luck
#' * birthEnvMu3: the contribution to LRO mu3 from birth
#'   environment luck * birthStateVar: the contribution to Var(LRO) from birth
#'   state luck
#' * birthStateMu3: the contribution to LRO mu3 from birth
#'   state luck
#' * survTrajecVar: a vector whose jth entry contains the
#'   contribution to Var(LRO) from survival trajectory luck at age j-1.
#' * survTrajecMu3: a vector whose jth entry contains the
#'   contribution to LRO mu3 from survival trajectory luck at age j-1.
#' * growthTrajecVar: a vector whose jth entry contains the
#'   contribution to Var(LRO) from growth trajectory luck at age j-1.
#' * growthTrajecMu3: a vector whose jth entry contains the
#'   contribution to LRO mu3 from growth trajectory luck at age j-1.
#' * envTrajecVar: a vector whose jth entry contains the
#'   contribution to Var(LRO) from environment trajectory luck at age j-1.
#' * envTrajecMu3: a vector whose jth entry contains the
#'   contribution to LRO mu3 from environment trajectory luck at age j-1.
#' * fecVar: a vector whose jth entry contains the
#'   contribution to Var(LRO) from fecundity luck at age j-1.
#' * fecMu3: a vector whose jth entry contains the contribution
#'   to LRO mu3 from fecundity luck at age j-1.
#' * totVarCondX: a vector whose jth entry is Var(LRO | trait = j)
#' * totMu3CondX: a vector whose jth entry is the LRO mu3
#'   conditional on trait = j
#' * birthEnvVarCondX: a vector whose jth entry is the contribution of
#'   birth environment luck to Var(LRO | trait = j)
#' * birthEnvMu3CondX: a vector whose jth entry is
#'   the contribution of birth environment luck to LRO mu3 conditional on
#'   trait = j
#' * birthStateVarCondX: a vector whose jth entry is the contribution
#'   of birth state luck to Var(LRO | trait = j)
#' * birthStateMu3CondX: a vector whose jth entry is the
#'   contribution of birth state luck to LRO mu3 conditional on trait = j
#' * survTrajecVarCondX: a matrix whose i,jth entry contains the
#'   contribution to Var(LRO | trait = i) from survival trajectory luck at age
#'   j-1.
#' * survTrajecMu3CondX: a matrix whose i,jth entry contains the
#'   contribution to LRO mu3 conditional on trait = i from survival
#'   trajectory luck at age j-1.
#' * growthTrajecVarCondX: a matrix whose i,jth entry contains
#'   the contribution to Var(LRO | trait = i) from growth trajectory luck at age
#'   j-1.
#' * growthTrajecMu3CondX: a matrix whose i,jth entry contains
#'   the contribution to LRO mu3 conditional on trait = i from growth
#'   trajectory luck at age j-1.
#' * envTrajecVarCondX: a matrix whose i,jth entry contains the
#'   contribution to Var(LRO | trait = i) from environment trajectory luck at
#'   age j-1.
#' * envTrajecMu3CondX: a matrix whose i,jth entry contains the
#'   contribution to LRO mu3 conditional on trait = i from environment
#'   trajectory luck at age j-1.
#' * fecVarCondX: a matrix whose i,jth entry contains the contribution
#'   to Var(LRO | trait = i) from fecundity luck at age j-1.
#' * fecMu3CondX: a matrix whose i,jth entry contains the
#'   contribution to LRO mu3 conditional on trait = i from fecundity luck
#'   at age j-1.
#' * lifespanCondX: a vector whose jth entry is the age by which
#'   survThreshold proportion of a cohort with trait j would be dead
#' @importFrom Matrix bdiag
...
#'       FlistAllTraits, Q, c0, traitDist)
partitionVarMu3EnvVarAndTraits = function (PlistAllTraits,
                                                FlistAllTraits, Q,
                                                c0, traitDist,
                                                maxAge=100,
                                                survThreshold=0.05,
  ...
  lifespanCondX = birthEnvMu3CondX =
    birthEnvVarCondX = birthStateMu3CondX =
    birthStateVarCondX = numeric (numTraits)
  expRCondX = totVarCondX = totMu3CondX = numeric (numTraits)

  survTrajecVarCondX = growthTrajecVarCondX = envTrajecVarCondX = fecVarCondX =
    matrix (0, numTraits, maxAge)
  survTrajecMu3CondX = growthTrajecMu3CondX = envTrajecMu3CondX =
    fecMu3CondX = matrix (0, numTraits, maxAge)
  ...
  for (x in 1:numTraits) {
    ...
    if (postBreedingCensus) {
      stop ("Post-breeding census not implemented yet.")
    } else {
      out = partitionVarMu3EnvVar (Plist=Plist, Flist=Flist,
                                        Q=Q, c0=c0, maxAge=maxAge,
                                        survThreshold=survThreshold,
                                        Fdist=Fdist)
    }

    birthEnvMu3CondX[x] = out$birthEnvMu3
    survTrajecMu3CondX[x,] = out$survTrajecMu3
    growthTrajecMu3CondX[x,] = out$growthTrajecMu3
    envTrajecMu3CondX[x,] = out$envTrajecMu3
    fecMu3CondX[x,] = out$fecMu3
    totMu3CondX[x] = out$totMu3
    surv = out$surv
    lifespanCondX[x] = out$lifespan
  ...
  ## Average over traits
  birthStateMu3 = traitAve (birthStateMu3CondX, traitDist)
  birthEnvMu3 = traitAve (birthEnvMu3CondX, traitDist)
  fecMu3 = apply (fecMu3CondX, 2, traitAve, traitDist)
  survTrajecMu3 = apply (survTrajecMu3CondX, 2, traitAve, traitDist)
  growthTrajecMu3 = apply (growthTrajecMu3CondX, 2, traitAve, traitDist)
  envTrajecMu3 = apply (envTrajecMu3CondX, 2, traitAve, traitDist)
  ...
  ## To get total mu3 or variance, we need to create transition
  ## matrices and reward matrices for a model in which state is
  ## classified by trait x stage x env.  I'll use the prefix "ts" to
  ## suggest "triple state" as a way to distinguish these matrices.
  ##
  ...
  mu3FromTraits = totMu3 - traitAve (totMu3CondX, traitDist)
  varFromTraits = totVar - traitAve (totVarCondX, traitDist)
  ...
  return (out=list(varFromTraits=varFromTraits,
                   mu3FromTraits=mu3FromTraits,
                   totVar=totVar, totMu3=totMu3,
                   birthEnvVar=birthEnvVar,
                   birthEnvMu3=birthEnvMu3,
                   birthStateVar=birthStateVar,
                   birthStateMu3=birthStateMu3,
                   ...
                   fecMu3=fecMu3,
                   totVarCondX=totVarCondX,
                   totMu3CondX=totMu3CondX,
                   birthEnvVarCondX=birthEnvVarCondX,
                   birthEnvMu3CondX=birthEnvMu3CondX,
                   birthStateVarCondX=birthStateVarCondX,
                   birthStateMu3CondX=birthStateMu3CondX,
                   survTrajecVarCondX=survTrajecVarCondX,
                   survTrajecMu3CondX=survTrajecMu3CondX,
                   growthTrajecVarCondX=growthTrajecVarCondX,
                   growthTrajecMu3CondX=growthTrajecMu3CondX,
                   envTrajecVarCondX=envTrajecVarCondX,
                   envTrajecMu3CondX=envTrajecMu3CondX,
                   fecVarCondX=fecVarCondX,
                   fecMu3CondX=fecMu3CondX,
                   lifespanCondX=lifespanCondX))
}
...
#'   "Poisson" and "Bernoulli".  Optional.  The default value is "Poisson".
#'
#' @details A post-breeding census is assumed: reproduction happens after
#'   survival and growth.  The details of this calculation, including the
#'   definitions of the extended states, can be found in Robin E. Snyder and
#'   Stephen P. Ellner.  2024. "To prosper, live long: Understanding the sources
#'   of reproductive skew and extreme reproductive success in structured
...
#'
#' @return A list containing the following: * birthStateVar: the contribution to
#'   Var(LRO) from birth state luck * survTrajecVar: a vector whose jth entry
#'   contains the contribution to Var(LRO) from survival trajectory luck at age
#'   j-1.  * growthTrajecVar: a vector whose jth entry contains the contribution
#'   to Var(LRO) from growth trajectory luck at age j-1.  * fecVar: a vector
#'   whose jth entry contains the contribution to Var(LRO) from fecundity luck
#'   at age j-1.  * totVar: the total variance in LRO * birthStateSkewness: the
#'   contribution to LRO skewness from birth state luck * survTrajecSkewness: a
#'   vector whose jth entry contains the contribution to LRO skewness from
#'   survival trajectory luck at age j-1.  * growthTrajecSkewness: a vector
#'   whose jth entry contains the contribution to LRO skewness from growth
#'   trajectory luck at age j-1.  * fecSkewness: a vector whose jth entry
#'   contains the contribution to LRO skewness from fecundity luck at age j-1.
#'   * totSkewness: the total skewness in LRO * lifespan: the age by which a
#'   proportion survThreshold of a cohort will be dead
#' @export
#'
#' @examples
...
partitionVarSkewnessNoEnvVarPostBreeding = function (Umat, Fmat, c0, maxAge=100,
                                                     survThreshold=0.05,
                                                     Fdist="Poisson",
                                                     esR1=NULL, esR2=NULL,
                                                     esR3=NULL,
  ...
  growthUpdateSkewness[1] = esSkewnessVec %*%
    c(mzZero, mzZero, Umat %*% PaC0)
  fecUpdateSkewness[1] = esSkewnessVec %*%
    c(Umat %*% PaC0, mzZero, mzZero)

  survUpdateVar[1] = esMu2Vec %*% c(mzZero, Sbullet %*% PaC0, mzZero)
  growthUpdateVar[1] = esMu2Vec %*% c(mzZero, mzZero, Umat %*% PaC0)
  fecUpdateVar[1] = esMu2Vec %*% c(Umat %*% PaC0, mzZero, mzZero)
  ...
  for (a in 2:maxAge) {
    ...
    growthUpdateVar[a] = esMu2Vec %*% c(mzZero, mzZero, Umat %*% PaC0)
    fecUpdateVar[a] = esMu2Vec %*% c(Umat %*% PaC0, mzZero, mzZero)
...
#'
#' Partitions variance and skewness of lifetime reproductive output (LRO) for
#' models with environmental variation but without trait variation, assuming a
...
#'   contains the contribution to LRO skewness from survival trajectory luck at
#'   age j-1.
#' * growthTrajecSkewness: a vector whose jth entry contains the
#'   contribution to LRO skewness from growth trajectory luck at age j-1.
#' * envTrajecSkewness: a vector whose jth entry contains the
