...
#' Calculates the mean, the 2nd and 3rd non-central moments, and the
#' skewness of a lifetime reward
#'
#' Given a transition matrix and appropriate reward matrices,
#' calculates the mean, the 2nd and 3rd non-central moments, and the
...
#' @param R1 A matrix.  R1\[i,j\] is the expected reward
#'   (e.g. reproductive output) that occurs when making a transition
#'   from state j to state i.  Reward matrices include the "dead"
#'   state: if there are n states, R1 should be size n+1 x n+1.
#' @param R2 A matrix.  R2\[i,j\] is the expected squared reward
#'   (e.g. reproductive output) that occurs when making a transition
#'   from state j to state i.  Reward matrices include the "dead"
#'   state: if there are n states, R2 should be size n+1 x n+1.
#' @param R3 A matrix.  R3\[i,j\] is the expected cubed reward
#'   (e.g. reproductive output) that occurs when making a transition
#'   from state j to state i.  Reward matrices include the "dead"
#'   state: if there are n states, R3 should be size n+1 x n+1.
#' @return A list containing
...
#'   (variance) of lifetime reward for a trajectory starting in state
#'   j 
...
#'   central moment / variance^3/2) of lifetime reward for a
#'   trajectory starting in state j
#' @examples
