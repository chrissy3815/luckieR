...
distLifespanCondR2PostBreeding = function (Plist, Flist, Q,
  ...
  ## Fbullet updates number of kids.  Note: in a post-breeding census
  ## model, Fbullet acts after Sbullet, so we're only updating the
  ## #kids of survivors, but the entries of F represent fecundity
  ## averaged over parents who survived to reproduce and those who did
  ## not, so here we need fecundity conditioned on survival.
  surv = colSums(M)
  for (j in 1:mT) {
    for (i in j:(mT-1)) {
      for (z in 1:bigmz) {
        if (Fdist == "Poisson") {
          Fbullet[(i-1)*bigmz + z, (j-1)*bigmz + z] =
            ifelse(surv[z] == 0, 0,
                   dpois (i-j, lambda=sum(F[,z])/surv[z]))
        } else if (Fdist == "Bernoulli") {
          Fbullet[(i-1)*bigmz + z, (j-1)*bigmz + z] =
            ifelse (surv[z] == 0, 0,
                    dbinom (i-j, prob=sum(F[,z]/surv[z]), size=1))
        } else {
          stop ("Supported options for Fdist are 'Poisson' and 'Bernoulli'.")
        }
      }
  ...
  if (sum(abs(range(Qbullet %*% Fbullet %*%  Gbullet %*% Sbullet - A)))
      > epsilon)
    ## warning("Qbullet %*% Fbullet %*% Gbullet %*% Sbullet differs
    ## substantially from A")
    cat("Qbullet %*% Fbullet %*% Gbullet %*% Sbullet differs substantially from A")
  ...
  ##esbigF = esA = matrix (0, esmzA, esmzA)
  esA = matrix (0, esmzA, esmzA)
  esA[mzA + 1:mzA, 1:mzA] = Sbullet
  esA[2*mzA + 1:mzA, mzA + 1:mzA] = Gbullet
  esA[3*mzA + 1:mzA, 2*mzA + 1:mzA] = Fbullet
  esA[1:mzA, 3*mzA + 1:mzA] = Qbullet
