...
## Juveniles mature with prob. 0.5 and die otherwise.  Adults do not
## survive to the following year.
P1 = matrix (c(0, 0.5, 0, 0), 2, 2)
## If you lived to be an adult, you have a mean of 1 offspring.  I
## therefore expect the LRO dist. to be dpois(0:10, lambda=1) * 0.5.
## But it's not.  (BUG)
F1 = matrix (c(0, 0.5, 0, 0), 2, 2)
Plist = list (P1, P1)
Flist = list (F1, F1)
Q = matrix (1/2, 2, 2)
c0 = c(1,0)
...
if (FALSE) {
  ...
  ## The columns of B should sum to 1 and they do.
  B = mk_B (F1, maxClutchSize, Fdist)

  ## Construct A, the transition matrix for a (number of kids) x stage x
  ## env. model.
  out = make_AxT (B, P1, mT)
  A = out$A
  mzA = mz*mT
