...
#############################################################################################
### Computes R0 and the total elasticities of R0 to survival and fecundity. 
### offspring_weighting determines if we normalize fecundity based on survival to maturity. 
### Total elasticity to F should equal 1 -- x% increase in F gives x% increase in R0.  
### Why did I not realize this until it popped out as the answer? 
#############################################################################################
R0_and_elas = function(Umat, Fmat, offspring_weighting=FALSE, repro_var="poisson") {
  ...
  # If we are using offspring weights, rescale F:
  if(offspring_weighting) {
    offspring_weight = prob_repro_wt(Umat, Fmat, repro_var=repro_var)
    Fmat<- sweep(Fmat, MARGIN = 1, offspring_weight, FUN="*")
  }
  
  ### reuse code from the IPM book 
  P = Umat; F = Fmat; K=P+F; 
  IPM.eig.sys <- eigen(K)
  ...
  ## sensitivity matrix of NGM M 
  NGM.eig.sys <- eigen(M)  
  R0 <- Re(NGM.eig.sys$values[1]); 
  w.z <- Re(NGM.eig.sys$vectors[,1])
  v.z1 <- Re(eigen(t(M))$vectors[,1])
  NGM.M.sens <- outer(v.z1, w.z, "*")/sum(v.z1 * w.z)
  
  NGM.F.sens = NGM.P.sens = NGM.F.elas = NGM.P.elas = matrix(NA,mz,mz); 
  Delta0 = matrix(0,mz,mz); 
  for(i in 1:mz) {
    for(j in 1:mz) {
      Delta_ij = Delta0; Delta_ij[i,j]=1; 
      NGM.F.sens[i,j] = sum(NGM.M.sens*(Delta_ij%*%N))
      NGM.F.elas[i,j] = F[i,j]*NGM.F.sens[i,j]/R0
      
      NGM.P.sens[i,j] = sum(NGM.M.sens*(F%*%N%*%Delta_ij%*%N)) 
      NGM.P.elas[i,j] = P[i,j]*NGM.P.sens[i,j]/R0
    }} 
  
  #### Do it again numerically 
  Fplus = 1.001*F; Mplus = Fplus%*%N; 
  R0plus = eigen(Mplus)$values[1]; 
  F_elas_finite = 1000*(R0plus - R0)/R0; 
  
  Pplus = 1.001*P; Mplus = F%*%solve(diag(mz) - Pplus); 
  R0plus = eigen(Mplus)$values[1]; 
  P_elas_finite = 1000*(R0plus - R0)/R0
  
  
  return(c(F_elas = sum(NGM.F.elas), P_elas = sum(NGM.P.elas), R0 = R0,P_elas_finite=P_elas_finite,F_elas_finite=F_elas_finite)); 
  
}

#############################################################################################
### Computes lambda and the total elasticities of lambda to survival and fecundity. 
### offspring_weighting determines if we normalize fecundity based on survival to maturity. 
#############################################################################################
lambda_and_elas = function(Umat,Fmat, offspring_weighting=FALSE, repro_var="poisson",  testing=FALSE) {
  ...
  ## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
  ## kernel sensitivity and elasticities 
  ## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
  
  ## kernel sensitivity
  K.sens <- outer(v.z1, w.z, "*")/sum(v.z1 * w.z)
  
  ## kernel elasticity 
  K.elas <- K.sens * K/ lambda
  if(testing) cat("Total elasticity ", sum(K.elas), " should equal 1","\n"); 
  
  ## calculate the elasticities of the survival and reproduction components
  P.elas <- P * K.sens / lambda
  F.elas <- F * K.sens / lambda
  
  return(c(P_elas = sum(P.elas),F_elas=sum(F.elas),lambda=lambda)); 
}
...
plot_var_components_quartiles<- function(exes, meanz, lower_bar, upper_bar, ax.labz, legend_loc){
  ...
  legend(legend_loc, title = "Luck component",
         legend=c("Survival", "Growth", "Fecundity"), 
         col=colz[c(6,4,2)], lty=1, lwd=2)
}
